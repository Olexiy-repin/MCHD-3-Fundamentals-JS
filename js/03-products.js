...
const onProductCardClick = event => {
  ...
  const cardId = Number(productCard.dataset.id);

  const currentCardInfo = products.find(product => product.id === cardId);

  const modalInstance = basicLightbox.create(`
    <img class="product-modal-img" src="${currentCardInfo.imgUrl}" alt="${currentCardInfo.title}" />
    <div class="product-modal-text-content">
      <h2 class="product-modal-title">${currentCardInfo.title}</h2>
      <p class="product-modal-price">Price: ${currentCardInfo.price}uah.</p>
      <p class="product-modal-desc">${currentCardInfo.description}</p>
    </div>
    `);
